...
import { PLAYER_AVATARS, GameMode, Role, AdvancedRole } from "@/lib/gameTypes";
...
export const MultiplayerLobbyScreen = ({ gameMode, onBack }: MultiplayerLobbyScreenProps) => {
  const { 
    room, 
    players, 
    currentPlayer, 
  ...
  const handleStartGame = async () => {
    ...
    if (gameMode === "advanced") {
      ...
      const remainingSlots = players.length - advancedRoles.length;
    ...
    players.forEach((player, index) => {
      roleAssignments[player.id] = shuffledRoles[index];
    });

    await startGame(roleAssignments);
  };
  ...
  // If game is playing, show game screen
